namespace DiGi.Geometry.xUnit
{
    public partial class Facts
    {
        ...
        {
            DiGi.Geometry.Planar.Classes.Point2D point2D_Origin = new(0.0, 0.0);
            DiGi.Geometry.Planar.Classes.Vector2D vector2D_Direction = new(0.0, 1.0);
            DiGi.Geometry.Planar.Classes.Ray2D ray2D_Test = new(point2D_Origin, vector2D_Direction);

            // Test projection of point in front of the origin
            DiGi.Geometry.Planar.Classes.Point2D? point2D_ProjFront = ray2D_Test.Project(new DiGi.Geometry.Planar.Classes.Point2D(2.0, 10.0));
            Assert.NotNull(point2D_ProjFront);
            if (point2D_ProjFront is not null)
            {
                Assert.Equal(point2D_ProjFront, new DiGi.Geometry.Planar.Classes.Point2D(0.0, 10.0));
            }

            // Test projection of point behind the origin (should clamp to origin)
            DiGi.Geometry.Planar.Classes.Point2D? point2D_ProjBehind = ray2D_Test.Project(new DiGi.Geometry.Planar.Classes.Point2D(2.0, -10.0));
            Assert.NotNull(point2D_ProjBehind);
            if (point2D_ProjBehind is not null)
            {
                Assert.Equal(point2D_ProjBehind, new DiGi.Geometry.Planar.Classes.Point2D(0.0, 0.0));
            }

            // Test Distance method for point in front
            double double_DistFront = ray2D_Test.Distance(new DiGi.Geometry.Planar.Classes.Point2D(2.0, 10.0));
            Assert.Equal(2.0, double_DistFront, 9);

            // Test Distance method for point behind origin
            double double_DistBehind = ray2D_Test.Distance(new DiGi.Geometry.Planar.Classes.Point2D(2.0, -10.0));
            ...
            bool bool_OnRay = ray2D_Test.On(new DiGi.Geometry.Planar.Classes.Point2D(0.0, 5.0));
            Assert.True(bool_OnRay);

            // Test On method for point behind the ray's origin (should return false)
            bool bool_BehindRay = ray2D_Test.On(new DiGi.Geometry.Planar.Classes.Point2D(0.0, -5.0));
        ...
        {
            DiGi.Geometry.Planar.Classes.Point2D point2D_Origin = new(1.0, 2.0);
            DiGi.Geometry.Planar.Classes.Vector2D vector2D_Direction = new(1.0, 0.0);
