...
namespace DiGi.Geometry.xUnit
{
    public partial class Facts
    {
        ...
        public void VerticalPosition()
        {
            ...
            Assert.Equal(Core.Enums.VerticalPosition.Undefined, Planar.Query.VerticalPosition(segment2D_Null, point2D_Null));
            ...
            Assert.Equal(Core.Enums.VerticalPosition.Above, Planar.Query.VerticalPosition(segment2D_Horizontal, point2D_AboveSegment));

            // Point below
            Point2D point2D_BelowSegment = new(5.0, -1.0);
            Assert.Equal(Core.Enums.VerticalPosition.Below, Planar.Query.VerticalPosition(segment2D_Horizontal, point2D_BelowSegment));

            // Point on
            Point2D point2D_OnSegment = new(5.0, 2.0);
            Assert.Equal(Core.Enums.VerticalPosition.On, Planar.Query.VerticalPosition(segment2D_Horizontal, point2D_OnSegment));

            // Point outside X-bounds
            Point2D point2D_OutsideSegment = new(12.0, 5.0);
            Assert.Equal(Core.Enums.VerticalPosition.Undefined, Planar.Query.VerticalPosition(segment2D_Horizontal, point2D_OutsideSegment));
            ...
            Assert.Equal(Core.Enums.VerticalPosition.Inside, Planar.Query.VerticalPosition(segmentable2D_Polygon, point2D_InsidePolygon));

            // Point completely below the polygon
            Point2D point2D_BelowPolygon = new(5.0, -3.0);
            Assert.Equal(Core.Enums.VerticalPosition.Below, Planar.Query.VerticalPosition(segmentable2D_Polygon, point2D_BelowPolygon));

            // Point completely above the polygon
            Point2D point2D_AbovePolygon = new(5.0, 13.0);
            Assert.Equal(Core.Enums.VerticalPosition.Above, Planar.Query.VerticalPosition(segmentable2D_Polygon, point2D_AbovePolygon));

            // Point on the boundary of the polygon
            Point2D point2D_OnPolygon = new(5.0, 0.0);
            Assert.Equal(Core.Enums.VerticalPosition.On, Planar.Query.VerticalPosition(segmentable2D_Polygon, point2D_OnPolygon));
        }
    }
}