...
namespace DiGi.Geometry.xUnit
{
    public partial class Query
    {
        ...
        public void BoundingBox3D_InRange_SlabMethod()
        {
            ...
            Line3D line3D_Through = new Line3D(new Point3D(-10, 0, 0), new Vector3D(1, 0, 0));
            Assert.True(boundingBox3D.InRange(line3D_Through));

            // 2. Line missing AABB
            Line3D line3D_Miss = new Line3D(new Point3D(-10, 10, 0), new Vector3D(1, 0, 0));
            ...
            Ray3D ray3D_Inside = new Ray3D(new Point3D(0, 0, 0), new Vector3D(0, 0, 1));
            Assert.True(boundingBox3D.InRange(ray3D_Inside));

            // 6. Ray pointing towards AABB
            Ray3D ray3D_Towards = new Ray3D(new Point3D(-10, 0, 0), new Vector3D(1, 0, 0));
            Assert.True(boundingBox3D.InRange(ray3D_Towards));

            // 7. Ray pointing away from AABB
            Ray3D ray3D_Away = new Ray3D(new Point3D(-10, 0, 0), new Vector3D(-1, 0, 0));
        ...
        public void Polyhedron_IntersectionResult3D_Correctness()
        {
            ...
            Line3D line3D_Through = new Line3D(new Point3D(0, 0, -10), new Vector3D(0, 0, 1));
