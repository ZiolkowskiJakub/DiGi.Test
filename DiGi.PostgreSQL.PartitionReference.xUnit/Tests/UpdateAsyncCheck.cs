...
namespace DiGi.PostgreSQL.PartitionReference.xUnit
{
    public partial class Tests
    {
        ...
        public async Task UpdateAsyncCheck()
        {
            ...
            Classes.PartitionReference? partitionReference_1 = await partitionReferencePostgreSQLConverter.UpdateAsync(address_1);
            Assert.NotNull(partitionReference_1);

            Address? address_2 = await partitionReferencePostgreSQLConverter.GetSerializableObjects<Address>(partitionReference_1);
            Assert.NotNull(partitionReference_1);

            Assert.Equal(address_1.ToSystem_String(), address_2.ToSystem_String());

            Classes.PartitionReference? partitionReference_2 = await partitionReferencePostgreSQLConverter.RemoveAsync(partitionReference_1);
